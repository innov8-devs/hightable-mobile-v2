...
import 'package:hightable_mobile_v2/animations/animations_double.dart';
...
class AnimateInSequenceWithDuration extends StatefulWidget {
  ...
  @override
  State<AnimateInSequenceWithDuration> createState() =>
      _AnimateInSequenceWithDurationState();
}

class _AnimateInSequenceWithDurationState
    extends State<AnimateInSequenceWithDuration> with TickerProviderStateMixin {
  ...
  void init() {
    ...
    _animationControllers = widget.child.children
        .map((_) =>
            AnimationController(vsync: this, duration: widget.animateNextAfter))
        .toList();
    _animations = List.generate(widget.child.children.length, (index) {
      if (widget.animationType == AnimationType.scale) {
        return Tween(begin: 0.0, end: 1.0).animate(
          CurvedAnimation(
              parent: _animationControllers[index], curve: Curves.bounceOut),
        );
      }
                  ...
                  return Transform.translate(
                    offset: Offset(
                      ...
                      _animations[index].value.getNewRangeValue(
                            ...
                            newRange: DoubleRange(
                                min: widget.displacementOnYAxis, max: 0),
                          ),
                    ),
                    child: Opacity(
                        opacity: _animations[index].value, child: child),
                  );
